...
ERC20("Hermes Multi Staked ONE", "hmONE")
{
    ...
    event minDelegateChanged(uint256 oldFee, uint256 newFee);
    ...
    function changeMinDelegate(uint256 _minDelegate) external onlyOwner {
        require(
            _minDelegate > 0,
            "minimum delegate amount should be greater than zero."
        );
        emit minDelegateChanged(minDelegate, _minDelegate);
        minDelegate = _minDelegate;
    }
    ...
    function changeFeeAddress(address payable _address) external onlyOwner {
        require(_address != address(0x0), "invalid fee address");
        emit feeAddressChanged(feeAddress, _address);
        feeAddress = _address;
    }
    ...
    function unstake(uint256 _share) public {
        ...
        // Calculates the amount of ONE the sONE is worth
    ...
    function withdraw(uint256 _amount) public nonReentrant {
        (bool status, string memory reason) = canWithdraw(msg.sender, _amount);
